// ClawDB Node client for the sidecar mode (`clawdb serve <db> <socket>`).
...
class ClawDBError extends Error {
  constructor(code, message) {
    super(`[clawdb:${code}] ${message}`);
    this.code = code;
  }
}
ClawDBError.CONFLICT_RETRY = 9;
  ...
  constructor(socket) {
    ...
    rl.on('line', (line) => {
      ...
      try {
        ...
        else waiter.reject(new ClawDBError(response.code ?? 1, response.error ?? 'unknown'));
      } catch (e) {
        waiter.reject(e);
      }
    });
    socket.on('error', (e) => this._failAll(e));
    socket.on('close', () => this._failAll(new ClawDBError(1, 'sidecar closed the connection')));
  }
...
module.exports = { SidecarClient, ClawDBError, decodeValue };
