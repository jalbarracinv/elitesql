...
class EliteSQLError(Exception):
    ...
    IO = 1                 # I/O failure. Over the sidecar, also a lost connection:
                           # a commit that was already sent MAY be published.
    CORRUPT = 2
    CONFLICT_RETRY = 9     # optimistic conflict: safe to retry the whole transaction
    DATABASE_LOCKED = 10
    UNIQUE_VIOLATION = 11
    READ_ONLY = 13
    MEMORY_LIMIT = 16
    COMMIT_UNKNOWN = 17    # the write IS published; only its crash durability is
                           # unknown. Never retry blindly: verify, then decide.
    QUERY_INTERRUPTED = 18
    AUTH = 20              # sidecar: missing or wrong token
    TRANSACTION_EXPIRED = 21  # sidecar: rolled back at the deadline; retry as a whole

    @property
    def retry_safe(self) -> bool:
        """True when nothing of the failed unit of work can have been published."""
        return self.code in (self.CONFLICT_RETRY, self.TRANSACTION_EXPIRED)

    @property
    def maybe_published(self) -> bool:
        """True when the write may already be visible despite the error."""
        return self.code in (self.COMMIT_UNKNOWN, self.IO)
...
def _decode_value(v: Any) -> Any:
    if isinstance(v, dict) and "$t" in v:
        ...
        if t == "vector":
            return list(v["v"])
        if t == "json":
            # Large integers travel as exact text; Python ints are unbounded.
            return json.loads(v["text"]) if "text" in v else v["v"]
        if t == "float64":
            return float(v["repr"])
    return v
...
class EliteSQL:
    ...
    def __init__(self, path: str | os.PathLike, durability: Optional[str] = None,
                 ...
                 memory: Optional[dict[str, Any]] = None, full_fsync: bool = False):
        ...
        if read_only:
            opts["read_only"] = True
        if full_fsync:
            # macOS: flush the drive cache at every barrier (F_FULLFSYNC).
            opts["full_fsync"] = True
...
class SidecarClient:
    ...
    def _call(self, request: dict, _locked: bool = True) -> Any:
        ...
        # The initial auth handshake runs before this client is shared, and
        # transactions/cursors hold the lock for their whole lifetime.
        return self._exchange(request)

    def _exchange(self, request: dict) -> Any:
        """One request/response pair on the pinned connection.

        Every request carries an id the server echoes. A response with another
        id, a timeout, or a closed socket leaves the stream in an unknown
        position, so the connection is closed and the client refuses further
        use: consuming a late response as the answer to a *later* request
        would silently return the wrong data.
        """
        if self._broken is not None:
            raise EliteSQLError(EliteSQLError.IO, f"sidecar connection is unusable: {self._broken}")
        self._next_id += 1
        ...
        try:
            ...
            self._break(f"timed out waiting for the response to request {request_id}")
            raise EliteSQLError(
                EliteSQLError.IO,
                "sidecar request timed out; the connection was closed. A write that "
                "was already sent may have been published: verify before retrying",
            ) from error
        except OSError as error:
            self._break(str(error))
            raise EliteSQLError(EliteSQLError.IO, f"sidecar I/O error: {error}") from error
        if not line:
            self._break("server closed the connection")
            raise EliteSQLError(
                EliteSQLError.IO,
                "sidecar closed the connection; a write that was already sent may "
                "have been published: verify before retrying",
            )
        response = json.loads(line)
        if response.get("id") != request_id:
            self._break(f"response id {response.get('id')!r} does not match request {request_id}")
            raise EliteSQLError(EliteSQLError.IO, "sidecar response/request mismatch; connection closed")
