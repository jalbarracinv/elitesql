//! `clawdb` — the ClawDB command line.
//!
...
use clawdb_core::{jsonio, Db, DbOptions, Durability, QueryOutput, Value};

mod serve;

const USAGE: &str = "\
clawdb — a tiny operational database for AI-native apps

USAGE:
  clawdb query <db> <sql>            execute one SQL statement
  clawdb repl <db>                   interactive SQL shell
  clawdb tables <db>                 list tables with their schemas
  clawdb check <db>                  offline integrity check
  clawdb compact <db>                compact segments and vector indexes
  clawdb repair <src> <dst>          salvage records into a fresh database
  clawdb export <db> <table>         records as JSON lines on stdout
  clawdb import <db> <table>         JSON lines from stdin into a table
  clawdb serve <db> <socket-path>    sidecar server over a Unix socket
  clawdb version
...
fn run(mut args: Vec<String>) -> Result<(), String> {
    ...
    match cmd.as_str() {
        ...
        "check" => {
            let [db_path] = take::<1>(&args)?;
            let report = clawdb_core::check(&db_path).map_err(|e| e.to_string())?;
        ...
        "repair" => {
            ...
            let report = clawdb_core::salvage(&src, &dst).map_err(|e| e.to_string())?;
...
fn repl(path: &str, opts: DbOptions) -> Result<(), String> {
    ...
    eprintln!("clawdb {} — {} — .exit to quit", env!("CARGO_PKG_VERSION"), path);
...
fn import(db: &Db, table: &str) -> Result<(), String> {
    ...
    for (line_no, line) in stdin.lock().lines().enumerate() {
        ...
        let mut record = clawdb_core::Record::new();
        for (k, v) in obj {
            let value = if k == "id" {
                match v.as_str() {
