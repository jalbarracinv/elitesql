...
//! ~100 orders per user). Run with: cargo bench -p clawdb-core --bench sql

use std::hint::black_box;

use clawdb_core::{Db, DbOptions, Durability, QueryOutput};
...
fn build_dataset() -> (TempDir, Db) {
    ...
    db.query("CREATE TABLE users (name text NOT NULL, email text NOT NULL)").unwrap();
    ...
    for i in 0..USERS {
        let mut r = clawdb_core::Record::new();
        r.insert("id".into(), clawdb_core::Value::Text(format!("u-{i:06}")));
        r.insert("name".into(), clawdb_core::Value::Text(format!("user {i}")));
        r.insert("email".into(), clawdb_core::Value::Text(format!("user{i}@example.com")));
    ...
    for i in 0..ORDERS {
        ...
        r.insert(
            "user_id".into(),
            clawdb_core::Value::Text(format!("u-{:06}", i % USERS)),
        );
        r.insert("amount".into(), clawdb_core::Value::Int64((i % 997) as i64));
        r.insert("note".into(), clawdb_core::Value::Text(format!("order {i}")));
