...
//! CLAWDB_CRASH_ITERS (default 15; the Phase 1 acceptance runs use more).
...
use clawdb_core::{check, Column, ColumnType, Db, DbOptions, Durability, Record, TableSchema, Value};

const ENV_WORKER: &str = "CLAWDB_CRASH_WORKER_DIR";
...
fn kill9_recovers_to_last_committed_state() {
    ...
    let iterations: u32 = std::env::var("CLAWDB_CRASH_ITERS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(15);

    let tmp = tempfile::tempdir().unwrap();
    let db_dir = tmp.path().join("crash.clawdb");
