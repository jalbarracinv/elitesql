...
use clawdb_core::{Db, Error, QueryOutput, Record, Value};
use tempfile::TempDir;

fn new_db() -> (TempDir, Db) {
    let dir = tempfile::tempdir().unwrap();
    let db = Db::create(dir.path().join("dt.clawdb")).unwrap();
    db.query(
...
fn timestamp_accepts_datetime_literals() {
    let dir = tempfile::tempdir().unwrap();
    let db = Db::create(dir.path().join("ts.clawdb")).unwrap();
...
fn survives_reopen_and_compaction() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("dt.clawdb");
    {
