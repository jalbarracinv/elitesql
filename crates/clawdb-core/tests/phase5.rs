...
fn text_db() -> (TempDir, Db) {
    ...
    let db = Db::create(dir.path().join("t.clawdb")).unwrap();
...
fn text_index_tracks_updates_deletes_reopen_compaction() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("t.clawdb");
    let (a, b);
    {
...
fn quantized_index_keeps_recall_and_shrinks_dump() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let (dbq, data) = build(&dir.path().join("quant.clawdb"), true);
    ...
    drop(dbq);
    let (dbf, _) = build(&dir.path().join("full.clawdb"), false);
    drop(dbf);
    ...
    let quant = size_of(&dir.path().join("quant.clawdb"));
    let full = size_of(&dir.path().join("full.clawdb"));
    ...
    let db = Db::open(dir.path().join("quant.clawdb")).unwrap();
    let hits = db
...
fn blob_db(threshold: usize) -> (TempDir, Db) {
    ...
    let db = Db::create_with(dir.path().join("b.clawdb"), opts).unwrap();
    db.create_table(TableSchema::new(
        "files",
        vec![
...
fn blob_files(dir: &TempDir) -> usize {
    std::fs::read_dir(dir.path().join("b.clawdb/blobs"))
...
fn blob_gc_on_compaction_and_corruption_detection() {
    ...
    let chunk = std::fs::read_dir(dir.path().join("b.clawdb/blobs"))
        .unwrap()
        .flatten()
        .map(|e| e.path())
        .next()
...
fn read_only_reads_and_rejects_writes() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("ro.clawdb");
    {
        let db = Db::create(&path).unwrap();
    ...
    assert!(matches!(rows, clawdb_core::QueryOutput::Rows { .. }));
