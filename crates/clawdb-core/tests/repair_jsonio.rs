...
use clawdb_core::{jsonio, salvage, Db, Record, Value};
...
fn salvage_recovers_valid_prefix_and_reports_damage() {
    ...
    let src = dir.path().join("damaged.clawdb");
    let dst = dir.path().join("salvaged.clawdb");
    ...
    drop(db);
    let check = clawdb_core::check(&dst).unwrap();
...
fn salvage_preserves_updates_and_deletes_when_segments_are_intact() {
    let dir = tempfile::tempdir().unwrap();
    let src = dir.path().join("wal-damaged.clawdb");
    let dst = dir.path().join("salvaged2.clawdb");
...
fn salvage_refuses_existing_destination() {
    let dir = tempfile::tempdir().unwrap();
    let src = dir.path().join("src.clawdb");
    {
        let db = Db::create(&src).unwrap();
        db.query("CREATE TABLE t (a int64)").unwrap();
    }
    let dst = dir.path().join("dst.clawdb");
...
fn import_export_type_mapping() {
    ...
    use clawdb_core::ColumnType as T;
    let cases = vec![
