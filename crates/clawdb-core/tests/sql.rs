...
use clawdb_core::{Db, Error, QueryOutput, Value};
use tempfile::TempDir;

fn new_db() -> (TempDir, Db) {
    let dir = tempfile::tempdir().unwrap();
    let db = Db::create(dir.path().join("sql.clawdb")).unwrap();
    (dir, db)
}

fn seeded() -> (TempDir, Db) {
    let (dir, db) = new_db();
    db.query("CREATE TABLE users (name text NOT NULL, email text, age int64)").unwrap();
    db.query("CREATE UNIQUE INDEX ON users (email)").unwrap();
    db.query("CREATE TABLE orders (user_id text NOT NULL, amount int64, note text)").unwrap();
...
fn create_table_types_and_errors() {
    let (_d, db) = new_db();
    db.query(
        "CREATE TABLE t (b bool, i int64, f float64, s text NOT NULL, \
         bl blob, ts timestamp, j json)",
    )
    .unwrap();
    assert!(db.tables().contains(&"t".to_string()));

    assert_sql_err(&db, "CREATE TABLE bad (x int)", "use int64");
    ...
    assert_sql_err(&db, "CREATE TABLE bad (x text PRIMARY KEY)", "implicit 'id'");
    assert_sql_err(&db, "CREATE TABLE bad (x text DEFAULT 'y')", "DEFAULT");
    assert_sql_err(&db, "CREATE TABLE bad (x text UNIQUE)", "CREATE UNIQUE INDEX");
}
...
fn insert_returns_ids_and_is_atomic() {
    ...
    let QueryOutput::Inserted { ids } = out else { panic!("expected Inserted") };
    assert_eq!(ids.len(), 3);
...
fn insert_coercions_and_blob() {
    let (_d, db) = new_db();
    db.query("CREATE TABLE t (f float64, ts timestamp, j json, b blob)").unwrap();
    db.query(
        "INSERT INTO t (f, ts, j, b) VALUES (3, 1722000000000000, '{\"k\": [1, 2]}', X'DEADBEEF')",
    )
    .unwrap();
    let (_, r) = rows(db.query("SELECT f, ts, j, b FROM t").unwrap());
    assert_eq!(r[0][0], Value::Float64(3.0), "int literal coerced to float64");
    ...
    assert_sql_err(&db, "INSERT INTO t (f) VALUES ('text')", "not valid for column");
}
...
fn select_where_order_limit_offset() {
    let (_d, db) = seeded();
    let (cols, r) = rows(
        db.query("SELECT name, age FROM users WHERE age >= 25 ORDER BY age DESC").unwrap(),
    );
    assert_eq!(cols, vec!["name", "age"]);
    assert_eq!(texts(&r, 0), vec!["eva", "ana", "bob"]);

    let (_, r) = rows(
        db.query("SELECT name FROM users ORDER BY age ASC LIMIT 1 OFFSET 1").unwrap(),
    );
...
fn select_predicates() {
    ...
    assert_eq!(q("SELECT name FROM users WHERE NOT age = 30 ORDER BY name"), vec!["bob", "eva"]);
    ...
    assert_eq!(q("SELECT name FROM users WHERE age NOT IN (25, 41)"), vec!["ana"]);
    assert_eq!(q("SELECT name FROM users WHERE (age = 25 OR age = 30) AND name = 'bob'"), vec!["bob"]);
    ...
    assert_eq!(q("SELECT name FROM users WHERE email = 'ana@x.com'"), vec!["ana"]);
}
...
fn select_star_and_aliases() {
    ...
    let (cols, r) = rows(
        db.query("SELECT name AS who, age AS years FROM users WHERE id = 'u1'").unwrap(),
    );
...
fn right_join_preserves_unmatched_right() {
    ...
    assert_eq!(r.len(), 4);
    let orphan = r.iter().find(|row| row[1] == Value::Text("orphan".into())).unwrap();
...
fn join_star_qualifies_headers() {
    let (_d, db) = seeded();
    let (cols, _) = rows(
        db.query("SELECT * FROM users u JOIN orders o ON o.user_id = u.id LIMIT 1").unwrap(),
    );
    assert_eq!(
        cols,
        vec!["u.id", "u.name", "u.email", "u.age", "o.id", "o.user_id", "o.amount", "o.note"]
    );
}
...
fn chained_joins() {
    let (_d, db) = seeded();
    db.query("CREATE TABLE tags (order_id text NOT NULL, tag text)").unwrap();
    db.query("CREATE INDEX ON tags (order_id)").unwrap();
    // Tag ana's 100 order.
    let (_, r) = rows(db.query("SELECT id FROM orders WHERE amount = 100").unwrap());
    let oid = match &r[0][0] {
        Value::Text(s) => s.clone(),
        _ => unreachable!(),
    };
    db.query(&format!("INSERT INTO tags (order_id, tag) VALUES ('{oid}', 'vip')")).unwrap();
...
fn update_with_where_reports_count() {
    let (_d, db) = seeded();
    let out = db.query("UPDATE users SET age = 26 WHERE name = 'bob'").unwrap();
    ...
    assert_sql_err(&db, "UPDATE users SET age = age", "SET only accepts literal");
}
...
fn unsupported_features_fail_with_clear_errors() {
    let (_d, db) = seeded();
    assert_sql_err(&db, "SELECT * FROM users FULL OUTER JOIN orders ON o.a = u.b", "FULL OUTER");
    ...
    assert_sql_err(&db, "SELECT name FROM users WHERE age + 1 = 31", "arithmetic");
    assert_sql_err(&db, "SELECT name FROM users WHERE name LIKE 'a%'", "LIKE");
    assert_sql_err(&db, "SELECT name FROM users WHERE age BETWEEN 20 AND 30", "BETWEEN");
    assert_sql_err(&db, "DROP TABLE users", "DROP");
    assert_sql_err(&db, "ALTER TABLE users", "ALTER");
    ...
    assert_sql_err(&db, "SELECT name FROM users; SELECT name FROM users", "one statement");
    assert_sql_err(&db, "INSERT INTO users VALUES ('x')", "column list");
...
fn sql_and_api_interoperate() {
    ...
    let mut rec = clawdb_core::Record::new();
    ...
    db.query(&format!("UPDATE users SET age = 51 WHERE id = '{id}'")).unwrap();
...
fn sql_survives_reopen() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("sql.clawdb");
    {
        let db = Db::create(&path).unwrap();
        db.query("CREATE TABLE kv (v text)").unwrap();
        db.query("INSERT INTO kv (id, v) VALUES ('k1', 'hello')").unwrap();
    }
